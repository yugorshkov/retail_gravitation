...
def get_osm_data(city: dict, local_prefix: str = "data") -> str:
    ...
    и фильтруем типы объектов карты
    """
    os.system(
...
def calculate_stores_area(filename: str) -> GeoDataFrame:
    """Вычисляем площадь каждого магазина города, как часть площади здания в
    котором он располагается. Логика определения площади следующая:
    1. Если в данных OSM геометрия магазина представлена полигоном, считаем,
        что магазин занимает всю влощадь полигона.
    2. Для магазинов, которые представлены точкой, находим геометрию здания,
        которая включает в себя эту точку. Вычисляем площадь здания.
    ...
    4. Для оставшихся магазинов принимаем площадь равной площади здания с
        поправкой на коэффициент доли площади магазина в здании:
